 <x-layouts.admin title="Edit Reservation">
             ...
             <form method="POST" action="{{ route('admin.reservations.update', $reservation) }}"
                 ...
                 <div class="space-y-10">
                     ...
                     <div class="grid grid-cols-1 md:grid-cols-6 gap-6">
                         ...
                         <div class="col-span-6 md:col-span-3">
                             <x-inputs.input label="Name client" name="name_client" id="name_client"
                                 value="{{ old('name_client', $reservation->name_client) }}" required />
                         </div>
                         ...
                         <div class="col-span-6 md:col-span-3">
                             <x-inputs.input name="phone" label="Phone" id="phone"
                             ...
                             <div>
                                 ...
                                 <select name="room_id" id="room_id" x-model="roomId">
                                     ...
                                     @foreach ($rooms as $key => $room)
                                         <option value="{{ $key }}"
                                             {{ $key === old('room_id', $reservation->room_id) ? 'selected' : '' }}>
                                             {{ $room }}
                                         </option>
                                     @endforeach
                                 </select>
                             ...
                             <div x-show="roomId" x-cloak>
                                 ...
                                 <div class="flex items-center gap-2">
                                     ...
                                     <input type="hidden" name="date_checkin" id="date_checkin" x-ref="checkin"
                                         value="{{ old('date_checkin', $reservation->date_checkin) }}">
                                     <input type="hidden" name="date_checkout" id="date_checkout" x-ref="checkout"
                                         value="{{ old('date_checkout', $reservation->date_checkout) }}">
                                 </div>
                         ...
                         <div class="col-span-6">
                             <x-inputs.textarea name="messages" label="Messages"
                                 id="messages">{{ old('messages', $reservation->messages) }}</x-inputs.textarea>
                         </div>
                     </div>
                     ...
                     <div class="flex flex-col-reverse md:flex-row items-center justify-end gap-4">
                         ...
                         <x-btn type="submit">Edit Reservation</x-btn>
                     </div>
                 </div>
             </form>
     ...
     <script>
         function reservationForm() {
             return {
                 reservationId: @js($reservation->id),
                 roomId: @js(old('room_id', $reservation->room_id)),
                 ...
                 init() {
                     ...
                     const oldCheckin = @js(old('date_checkin', $reservation->date_checkin));
                     const oldCheckout = @js(old('date_checkout', $reservation->date_checkout));
                 ...
                 async fetchBookedDates() {
                     ...
                     try {
                         const response = await fetch(`/api/rooms/${this.roomId}/booked/${this.reservationId}`);
                         const data = await response.json();

                         // data deve essere array di array: [['2025-06-24', '2025-06-26'], ...]
                         ...
                         console.error('Errore nel fetch delle date:', e);
                     }
                 }
             }
         }
